...
ThemeDist 是每日自动轮换的主题分发 API。主题由 CDN 边缘函数实时计算，无需定时构建。
          客户端 GET /api/v1/today.json，将返回的 34 个 CSS 自定义属性注入 :root，即可获得当日主题。
          同时提供社区主题（用户投稿 / 点赞 / 分享）和 AI 生成 等完整 API。
          无认证（公开端点），支持 CORS 跨域。写操作（投稿/点赞）设有滑动窗口限流。
  147+ 预设 农历 + 公历节日 CORS 跨域 CDN 友好 特效动画
...
方法 路径 说明    GET /api/v1/today.json 今日主题的完整数据（CSS 变量 + 特效 + 目录）   GET /api/v1/today.css 今日主题纯 CSS（:root { 变量），支持 <link> 阻塞引入消除 FOUC   GET /api/v1/today-safe 安全主题代理 — Vercel 优先，Netlify 备用，XSS 清洗后输出   GET /api/v1/theme/{preset}.json 指定预设的完整数据（147 个端点，永久缓存）   GET /api/v1/date=MM-DD 按日期查询主题，格式 MM-DD（如 /api/v1/date=02-14）  COMMUNITY 社区主题  GET /api/v1/diy/themes.json 社区主题列表（分页 + 排序 + 标签筛选）   GET /api/v1/diy/theme.json?id= 单个社区主题详情（含点赞数）   POST /api/v1/diy/submit.json 提交社区主题（进入审核队列）   POST /api/v1/diy/like.json 点赞社区主题（IP+UA 去重）  AI 生成  POST /api/v1/ai/generate.json AI 主题生成（规则引擎，或客户端 DeepSeek 直调）  高级功能 (NEW)  GET /api/v1/tokens.json W3C DTCG 设计令牌导出（color / typography / space / effects 层级结构）   GET /api/v1/today/favicon.svg 动态 Favicon（主色圆角矩形 + Logo 首字母）   GET /api/v1/today/fonts.css 自动字体注入（提取主题字体 → Google Fonts @import）   GET /api/v1/search/color.json?hex=&limit= 颜色相似度搜索（RGB 欧几里得距离排序）   GET /api/v1/recommend/{preset}.json 智能推荐引擎（Jaccard 标签 + 颜色距离加权）   GET /api/v1/trending.json 趋势排行榜（热度 = 点赞×10 + 使用量×1）   GET /api/v1/badge/{username}.svg GitHub 动态徽章（shields.io 风格作者统计）   POST /api/v1/ai/describe.json AI 逆向描述（分析 CSS 变量→中文风格描述）   POST /api/v1/telemetry/hit 匿名遥测上报（HyperLogLog 独立域名 + 主题使用计数）   POST /api/v1/pool/create.json 创建自定义轮换池   GET /api/v1/pool/{poolId}.json 自定义轮换池每日轮换查询  主题诊断与导出 (NEW)  GET /api/v1/theme/{id}/wcag.json WCAG 无障碍诊断（对比度评估、合规性检查）   GET /api/v1/theme/{id}/scale.json Tailwind 风格色阶生成（50~950，含 primary/secondary/accent/bg）   GET /api/v1/theme/{id}/export/shadcn.css Shadcn UI 适配器（HSL 变量 + 前景色自动推断）   POST /api/v1/extract-theme.json 图片取色（K-means 聚类 + UI 语义映射，零原生依赖）   GET /api/v1/theme/{id}/shiki.json Shiki / VS Code 代码高亮主题（TextMate Token 颜色映射）   GET /api/v1/today/pattern.css 动态 SVG 背景纹理（主题色几何图案，可作 CSS background-image）   GET /api/v1/today/weather.js 天气粒子渲染脚本（云/雨/雪/太阳/闪电），<script src> 引入即可自动渲染  展厅  PAGE /lab 全场景 API 展厅 — 5 大模块联动演示天气/纹理/高亮/缓动/WCAG  环境感知  GET /api/v1/weather-theme.json 天气自适应主题（基于 IP 经纬度 + Open-Meteo 免费天气 API）   GET /api/v1/status-override.json?status= 系统状态覆盖主题（maintenance / mourning / incident）  便捷工具  GET /api/v1/tailwind-config.json Tailwind CSS 配置片段生成（含 RGB 通道 + <alpha-value>）   GET /api/v1/today/palette.svg 今日主题调色盘 SVG 徽章（可嵌入 README / 博客 Footer）   GET /api/v1/theme/random.json 随机主题（支持 ?pool=static|community|all 和 ?seed=N）   GET /api/v1/theme/{id}/og.svg OG 社交分享卡片（1200×630 SVG，含主题色板与名称）


## 查询参数

参数 适用于 说明    ?tz= today.jsontoday.css 指定时区计算"今天"的日期。如 ?tz=America/New_York、?tz=Asia/Shanghai。无效时区自动回退到服务器时间。   ?overrides= today.jsontoday.css 微调 CSS 变量值。格式：--key:val;--key2:val2。如 ?overrides=--radii:0px;--font-body:monospace。最多 20 对，自动 XSS 清洗。


## 响应结构 /api/v1/today.json

字段 类型 说明   datestringISO 日期 YYYY-MM-DD（UTC 时区） generatedAtstringISO 8601 生成时间戳 presetstring预设标识符 (kebab-case) presetNamestring人类可读名称 cssVarsobject34 个 CSS 自定义属性键值对（5 组：Colors / Typography / Spacing / Effects / Ambient）。颜色变量自动附带 -rgb 通道变体 customCssstring主题专属 CSS（动画 keyframes、特效样式）。无自定义 CSS 时为空字符串 "" extensionsarray声明式装饰元素数组，支持 floating 和 decorative 两种类型。无扩展时为空数组 []。最多 20 个。 logoTextstring主题 Logo 文字标识 logoColorsarrayLogo 渐变色 hex 字符串数组。无 Logo 颜色时为空数组 [] availablenumber可用主题总数 directoryarray轻量索引 [{preset, name, primary, accent, logoText{'}\}'}] dailyIsCommunitybooleantrue 表示今日主题来自社区投稿 apiVersionstringAPI 版本号，当前为 "v1"

/api/v1/theme/{preset}.json 结构一致，但不含 date、generatedAt、available、directory。 示例响应 {
  "date": "2026-05-25",
  "preset": "holiday-145",
  "presetName": "DONT PANIC",
  "generatedAt": "2026-05-25T15:56:18.568Z",
  "cssVars": {
    "--color-primary": "#000080",
    "--color-secondary": "#4285F4",
    "--color-accent": "#4285f4",
  ...
  "customCss": "(theme-specific CSS animations)",
  "extensions": [
    {
      "type": "floating",
      "char": "🏮",
...
ThemeDist 提供官方轻量化 Web Component <themedist-runner>，将 CSS 变量注入全局 :root，
          并将装饰元素隔离在 Shadow DOM 中，防止样式污染宿主页面。内置 localStorage 缓存和网络降级。
 快速接入 <!-- 一行标签，自动完成 CSS 变量注入 + 装饰渲染 + 缓存降级 -->
<themedist-runner api="/api/v1/today.json" save-shadow="true"></themedist-runner>
<script src="https://themedist.netlify.app/sdk.js" defer></script>复制 属性    属性 默认值 说明   api/api/v1/today.json主题 API 端点 URL save-shadowtrue是否启用 Shadow DOM 隔离     安全设计  CSS 变量：注入到全局 :root，确保全站样式生效。 装饰元素：隔离在 Shadow DOM 中，通过 textContent 和 <template> 安全渲染，绝无 innerHTML 注入风险。 缓存降级：同日访问零网络开销，fetch 失败时自动使用过期缓存。
    轮换策略    # 来源 说明   1🌙 农历节日20+ 个中国传统节日（春节/元宵/端午/七夕/中秋等），每个有专属特效主题 2📅 公历节日元旦/情人节/Pi Day/圣帕特里克节/愚人节等 3🍗 Crazy Thu每周四特殊覆盖 4👥 社区池用户投稿主题每 3 天轮入一次（约 30% 天数），固定频率不受投稿量影响 5🎲 DailyPool147 预设按 dayOfYear % poolLength 轮换，全年无空档     缓存策略    端点 浏览器 CDN   /api/v1/today.json1 小时24 小时 (stale-while-revalidate) /api/v1/today.css1 小时24 小时 /api/v1/theme/*.json（预设）24 小时365 天 (immutable) /api/v1/theme/*/wcag.json1 小时— /api/v1/theme/*/scale.json365 天365 天 (immutable) /api/v1/weather-theme.json30 分钟— /api/v1/today/weather.js1 小时24 小时 /api/v1/diy/themes.json1 分钟— /api/v1/diy/theme.json5 分钟— POST /api/v1/diy/*不缓存— POST /api/v1/extract-theme.json不缓存— POST /api/v1/telemetry/hit不缓存— /api/v1/admin/*不缓存—    推荐客户端策略 
fetch → 解析 date → 存入 localStorage → 同日访问跳过网络请求 → 每人每天最多 1 次请求。
            Service Worker 可预缓存全部 147 个 /api/v1/theme/*.json（总计 ~500KB）。
...
从 ThemeDist 代理获取最新的 /api/v1/today.json 数据（Vercel 优先，Netlify 备用），并应用 XSS 清洗。
  ...
  curl curl https://themedist-monitor.vercel.app/api/v1/today-safe复制   响应结构    字段 类型 说明   datestringISO 日期 YYYY-MM-DD presetstring主题预设标识符 presetNamestring主题可读名称 authorstring主题作者（"themedist" 或社区作者名） availablenumber可用主题总数 cssVarsobjectCSS 自定义属性键值对（已完成 XSS 清洗） customCssstring主题专属 CSS（已完成 XSS 清洗） extensionsarrayHTML 装饰片段数组（已完成 XSS 清洗） directoryarray主题目录列表 _metaobject安全元信息：sanitized（是否已清洗）、schemaValid（结构校验）、timestamp（处理时间）    示例响应 {
  "date": "2026-05-24",
  "preset": "minimal-blue",
...
cssVars（34 个标准 CSS 变量）、customCss（自动生成的渐变背景）。
  代码高亮主题 (Shiki)  
...
export function useThemeDist() {
  ...
  "cssVars": {
    ...
    "--glass-blur": "blur(16px)",
    "--noise-opacity": "0",
    "--ambient-1": "rgba(66, 133, 244, 0.2)",
    "--ambient-2": "rgba(234, 67, 53, 0.1)",
    "--color-primary-rgb": "0, 0, 128",
    "--color-secondary-rgb": "66, 133, 244",
    "--color-accent-rgb": "66, 133, 244",
    "--color-bg-rgb": "0, 0, 26",
    "--color-surface-rgb": "15, 15, 41",
    "--color-text-rgb": "230, 230, 250",
    "--color-text-muted-rgb": "135, 206, 235",
    "--color-border-rgb": "66, 133, 244",
    "--ambient-1-rgb": "66, 133, 244",
    "--ambient-2-rgb": "234, 67, 53"
  },
  ...
  "extensions": [
    ...
    },
    {
      ...
      "fontSize": "50px",
      "opacity": 0.15,
      "animation": "floatGalaxy 20s linear infinite"
    },
    {
      "type": "floating",
      "char": "👍",
  ...
  "directory": [
    ...
    },
    {
      ...
      "name": "DRAGON BOAT (L05-05)",
      "primary": "#008b8b",
      "accent": "#00ced1",
      "logoText": "DRAGON BOAT"
    },
    {
      "preset": "holiday-l06-06",
      "name": "SUNNING BOOKS (L06-06)",
      "primary": "#b8860b",
      "accent": "#ffd700",
      "logoText": "SUNNING BOOKS"
    },
    {
      "preset": "holiday-l07-07",
      "name": "QIXI LOVE (L07-07)",
      "primary": "#8a2be2",
      "accent": "#9370db",
      "logoText": "QIXI LOVE"
    },
    {
      "preset": "holiday-l07-15",
      "name": "END OF HEAT (L07-15)",
      "primary": "#ff4500",
      "accent": "#ffa500",
      "logoText": "END OF HEAT"
    },
    {
      "preset": "holiday-l08-15",
      "name": "AUTUMN EQUINOX (L08-15)",
      "primary": "#d2691e",
      "accent": "#ffa500",
      "logoText": "AUTUMN EQUINOX"
    },
    {
      "preset": "holiday-l09-09",
      "name": "CHONGYANG (L09-09)",
      "primary": "#ff8c00",
      "accent": "#ff8c00",
      "logoText": "CHONGYANG"
    },
    {
      "preset": "holiday-l10-15",
      "name": "XIAYUAN FESTIVAL (L10-15)",
      "primary": "#000080",
      "accent": "#1e90ff",
      "logoText": "XIAYUAN FESTIVAL"
    },
    {
      "preset": "holiday-l12-08",
      "name": "MINOR COLD (L12-08)",
      "primary": "#4682b4",
      "accent": "#add8e6",
      "logoText": "MINOR COLD"
    },
    {
      "preset": "holiday-l12-23",
      "name": "MAJOR COLD (L12-23)",
      "primary": "#191970",
      "accent": "#b0c4de",
      "logoText": "MAJOR COLD"
    },
    {
      "preset": "holiday-l12-24",
      "name": "SOUTHERN XIAO NIAN (L12-24)",
      "primary": "#ff8c00",
      "accent": "#ffd700",
      "logoText": "SOUTHERN XIAO NIAN"
    },
    {
      "preset": "holiday-l12-30",
      "name": "NEW YEAR EVE (L12-30)",
      "primary": "#8b0000",
      "accent": "#ff0000",
      "logoText": "NEW YEAR EVE"
    },
    {
      "preset": "holiday-03-08",
      "name": "WOMEN POWER (03-08)",
      "primary": "#c71585",
      "accent": "#ff69b4",
      "logoText": "WOMEN POWER"
    },
    {
      "preset": "holiday-03-12",
      "name": "ARBOR DAY (03-12)",
      "primary": "#006400",
      "accent": "#228b22",
      "logoText": "ARBOR DAY"
    },
    {
      "preset": "holiday-05-01",
      "name": "MAY DAY 🍹 (05-01)",
      "primary": "#ff4b1f",
      "accent": "#00c6ff",
      "logoText": "MAY DAY 🍹"
    ...
    {
      "preset": "holiday-05-04",
      "name": "YOUTH DAY (05-04)",
      "primary": "#000080",
      "accent": "#4169e1",
      "logoText": "YOUTH DAY"
    },
    {
      "preset": "holiday-05-12",
      "name": "MOM’S LOVE (05-12)",
      "primary": "#ff69b4",
      "accent": "#ffb6c1",
      "logoText": "MOM’S LOVE"
    },
    {
      "preset": "holiday-06-01",
      "name": "CHILDREN'S DAY (06-01)",
      "primary": "#ff0000",
      "accent": "#ffd700",
      "logoText": "CHILDREN'S DAY"
    },
    {
      "preset": "holiday-07-01",
      "name": "PARTY FOUNDING DAY (07-01)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "PARTY FOUNDING DAY"
    },
    {
      "preset": "holiday-08-01",
      "name": "ARMY DAY (08-01)",
      "primary": "#004d00",
      "accent": "#ff0000",
      "logoText": "ARMY DAY"
    ...
    {
      "preset": "holiday-09-10",
      "name": "THANK YOU TEACHER (09-10)",
      "primary": "#4169e1",
      "accent": "#4169e1",
      "logoText": "THANK YOU TEACHER"
    },
    {
      "preset": "holiday-10-01",
      "name": "国庆快乐 (10-01)",
      "primary": "#EE1C25",
      "accent": "#ee1c25",
      "logoText": "国庆快乐"
    ...
    {
      "preset": "holiday-11-08",
      "name": "JOURNALISTS' DAY (11-08)",
      "primary": "#000080",
      "accent": "#4169e1",
      "logoText": "JOURNALISTS' DAY"
    },
    {
      "preset": "holiday-11-09",
      "name": "FIRE CONTROL DAY (11-09)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "FIRE CONTROL DAY"
    },
    {
      "preset": "holiday-12-13",
      "name": "NATIONAL MEMORIAL DAY (12-13)",
      "primary": "#000000",
      "accent": "#ffffff",
      "logoText": "NATIONAL MEMORIAL DAY"
    },
    {
      "preset": "holiday-02-02",
      "name": "WORLD WETLANDS DAY (02-02)",
      "primary": "#006400",
      "accent": "#2e8b57",
      "logoText": "WORLD WETLANDS DAY"
    },
    {
      "preset": "holiday-02-04",
      "name": "WORLD CANCER DAY (02-04)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "WORLD CANCER DAY"
    },
    {
      "preset": "holiday-04-07",
      "name": "HEALTH FIRST (04-07)",
      "primary": "#228b22",
      "accent": "#32cd32",
      "logoText": "HEALTH FIRST"
    },
    {
      "preset": "holiday-04-22",
      "name": "EARTH DAY (04-22)",
      "primary": "#228b22",
      "accent": "#228b22",
      "logoText": "EARTH DAY"
    },
    {
      "preset": "holiday-06-05",
      "name": "ECO MATRIX (06-05)",
      "primary": "#228b22",
      "accent": "#3cb371",
      "logoText": "ECO MATRIX"
    },
    {
      "preset": "holiday-06-08",
      "name": "OCEAN DAY (06-08)",
      "primary": "#000080",
      "accent": "#1e90ff",
      "logoText": "OCEAN DAY"
    },
    {
      "preset": "holiday-06-26",
      "name": "INTERNATIONAL DAY AGAINST DRUG ABUSE (06-26)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "INTERNATIONAL DAY AGAINST DRUG ABUSE"
    },
    {
      "preset": "holiday-09-27",
      "name": "WORLD TOURISM DAY (09-27)",
      "primary": "#ff4500",
      "accent": "#ff8c00",
      "logoText": "WORLD TOURISM DAY"
    },
    {
      "preset": "holiday-10-04",
      "name": "FAUNA MATRIX (10-04)",
      "primary": "#006400",
      "accent": "#228b22",
      "logoText": "FAUNA MATRIX"
    },
    {
      "preset": "holiday-12-10",
      "name": "HUMAN RIGHTS DAY (12-10)",
      "primary": "#000080",
      "accent": "#4169e1",
      "logoText": "HUMAN RIGHTS DAY"
    },
    {
      "preset": "holiday-03-07",
      "name": "GIRLS' DAY (03-07)",
      "primary": "#c71585",
      "accent": "#ff69b4",
      "logoText": "GIRLS' DAY"
    },
    {
      "preset": "holiday-05-20",
      "name": "520 LOVE DAY (05-20)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "520 LOVE DAY"
    },
    {
      "preset": "holiday-10-24",
      "name": "1024 GEEK DAY (10-24)",
      "primary": "#003300",
      "accent": "#00ff00",
      "logoText": "1024 GEEK DAY"
    },
    {
      "preset": "holiday-10-31",
      "name": "HALLOWEEN (10-31)",
      "primary": "#000000",
      "accent": "#ff8c00",
      "logoText": "HALLOWEEN"
    },
    {
      "preset": "holiday-12-25",
      "name": "CHRISTMAS (12-25)",
      "primary": "#006400",
      "accent": "#ff0000",
      "logoText": "CHRISTMAS"
    },
    {
      ...
      "name": "GONG XI FA CAI (L01-01)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "GONG XI FA CAI"
    },
    {
      "preset": "holiday-04-04",
      "name": "MEMORY & HOPE (04-04)",
      "primary": "#2e8b57",
      "accent": "#2e8b57",
      "logoText": "MEMORY & HOPE"
    },
    {
      "preset": "holiday-08-08",
      "name": "MEOW MATRIX (08-08)",
      "primary": "#ff9eaa",
      "accent": "#ffb3ba",
      "logoText": "MEOW MATRIX"
    },
    {
      "preset": "holiday-03-10",
      "name": "SUPER MATRIX (03-10)",
      "primary": "#e52521",
      "accent": "#e52521",
      "logoText": "SUPER MATRIX"
    },
    {
      "preset": "holiday-l02-15",
      "name": "SPRING EQUINOX (L02-15)",
      "primary": "#d2691e",
      "accent": "#ffa500",
      "logoText": "SPRING EQUINOX"
    },
    {
      "preset": "holiday-05-25",
      "name": "DONT PANIC (05-25)",
      "primary": "#000080",
      "accent": "#4285f4",
      "logoText": "DONT PANIC"
    },
    {
      "preset": "holiday-06-16",
      "name": "DAD YOU ROCK (06-16)",
      "primary": "#4169e1",
      "accent": "#4169e1",
      "logoText": "DAD YOU ROCK"
    },
    {
      "preset": "holiday-l01-07",
      "name": "REN RI FESTIVAL (L01-07)",
      "primary": "#ff8c00",
      "accent": "#ffd700",
      "logoText": "REN RI FESTIVAL"
    },
    {
      "preset": "holiday-l01-25",
      "name": "GRANARY FILLING (L01-25)",
      "primary": "#b8860b",
      "accent": "#d4af37",
      "logoText": "GRANARY FILLING"
    },
    {
      "preset": "holiday-l02-03",
      "name": "WENCHANG BIRTH (L02-03)",
      "primary": "#4169e1",
      "accent": "#4169e1",
      "logoText": "WENCHANG BIRTH"
    },
    {
      "preset": "holiday-l04-08",
      "name": "BUDDHA BIRTHDAY (L04-08)",
      "primary": "#b8860b",
      "accent": "#ffd700",
      "logoText": "BUDDHA BIRTHDAY"
    },
    {
      "preset": "holiday-l06-24",
      "name": "TORCH FESTIVAL (L06-24)",
      "primary": "#b22222",
      "accent": "#ff4500",
      "logoText": "TORCH FESTIVAL"
    },
    {
      "preset": "holiday-l10-01",
      "name": "WINTER BEGINS (L10-01)",
      "primary": "#191970",
      "accent": "#87ceeb",
      "logoText": "WINTER BEGINS"
    },
    {
      "preset": "holiday-l12-16",
      "name": "WEI YA FESTIVAL (L12-16)",
      "primary": "#b22222",
      "accent": "#ff4500",
      "logoText": "WEI YA FESTIVAL"
    },
    {
      "preset": "holiday-l02-01",
      "name": "AWAKENING OF INSECTS (L02-01)",
      "primary": "#ff8c00",
      "accent": "#ff8c00",
      "logoText": "AWAKENING OF INSECTS"
    },
    {
      "preset": "holiday-l03-01",
      "name": "PURE BRIGHTNESS (L03-01)",
      "primary": "#228b22",
      "accent": "#32cd32",
      "logoText": "PURE BRIGHTNESS"
    },
    {
      "preset": "holiday-l04-01",
      "name": "SUMMER BEGINS (L04-01)",
      "primary": "#ff8c00",
      "accent": "#ff8c00",
      "logoText": "SUMMER BEGINS"
    },
    {
      "preset": "holiday-l04-15",
      "name": "GRAIN BUDS (L04-15)",
      "primary": "#228b22",
      "accent": "#32cd32",
      "logoText": "GRAIN BUDS"
    },
    {
      "preset": "holiday-l05-15",
      "name": "SUMMER SOLSTICE (L05-15)",
      "primary": "#ff4500",
      "accent": "#ffd700",
      "logoText": "SUMMER SOLSTICE"
    },
    {
      "preset": "holiday-l06-01",
      "name": "MINOR HEAT (L06-01)",
      "primary": "#ff4500",
      "accent": "#ff8c00",
      "logoText": "MINOR HEAT"
    },
    {
      "preset": "holiday-l06-15",
      "name": "MAJOR HEAT (L06-15)",
      "primary": "#b22222",
      "accent": "#ff4500",
      "logoText": "MAJOR HEAT"
    },
    {
      "preset": "holiday-l07-01",
      "name": "AUTUMN BEGINS (L07-01)",
      "primary": "#d2691e",
      "accent": "#ff8c00",
      "logoText": "AUTUMN BEGINS"
    },
    {
      "preset": "holiday-l08-01",
      "name": "WHITE DEW (L08-01)",
      "primary": "#4682b4",
      "accent": "#ffffff",
      "logoText": "WHITE DEW"
    },
    {
      "preset": "holiday-l09-01",
      "name": "COLD DEW (L09-01)",
      "primary": "#191970",
      "accent": "#87ceeb",
      "logoText": "COLD DEW"
    },
    {
      "preset": "holiday-l09-15",
      "name": "FROST'S DESCENT (L09-15)",
      "primary": "#4682b4",
      "accent": "#ffffff",
      "logoText": "FROST'S DESCENT"
    },
    {
      "preset": "holiday-l11-01",
      "name": "HEAVY SNOW (L11-01)",
      "primary": "#191970",
      "accent": "#ffffff",
      "logoText": "HEAVY SNOW"
    },
    {
      "preset": "holiday-l11-15",
      "name": "WINTER SOLSTICE (L11-15)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "WINTER SOLSTICE"
    },
    {
      "preset": "holiday-01-26",
      "name": "COMIC DAY (01-26)",
      "primary": "#0000ff",
      "accent": "#ff0000",
      "logoText": "COMIC DAY"
    },
    {
      "preset": "holiday-02-10",
      "name": "WEATHER DAY (02-10)",
      "primary": "#4682b4",
      "accent": "#87ceeb",
      "logoText": "WEATHER DAY"
    },
    {
      "preset": "holiday-03-15",
      "name": "315 CONSUMER RIGHTS (03-15)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "315 CONSUMER RIGHTS"
    },
    {
      "preset": "holiday-03-22",
      "name": "WORLD WATER DAY (03-22)",
      "primary": "#000080",
      "accent": "#00bfff",
      "logoText": "WORLD WATER DAY"
    },
    {
      "preset": "holiday-04-02",
      "name": "AUTISM AWARENESS (04-02)",
      "primary": "#000080",
      "accent": "#4169e1",
      "logoText": "AUTISM AWARENESS"
    },
    {
      "preset": "holiday-04-26",
      "name": "IP DAY (04-26)",
      "primary": "#4169e1",
      "accent": "#4169e1",
      "logoText": "IP DAY"
    ...
    {
      "preset": "holiday-05-18",
      "name": "MUSEUM DAY (05-18)",
      "primary": "#3e1f00",
      "accent": "#8b4513",
      "logoText": "MUSEUM DAY"
    },
    {
      "preset": "holiday-05-22",
      "name": "BIODIVERSITY DAY (05-22)",
      "primary": "#006400",
      "accent": "#228b22",
      "logoText": "BIODIVERSITY DAY"
    },
    {
      "preset": "holiday-05-31",
      "name": "HARVEST FESTIVAL (05-31)",
      "primary": "#8b0000",
      "accent": "#d4af37",
      "logoText": "HARVEST FESTIVAL"
    },
    {
      "preset": "holiday-06-14",
      "name": "BLOOD DONOR DAY (06-14)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "BLOOD DONOR DAY"
    },
    {
      "preset": "holiday-07-02",
      "name": "UFO DAY (07-02)",
      "primary": "#4b0082",
      "accent": "#00ff00",
      "logoText": "UFO DAY"
    ...
    {
      "preset": "holiday-07-17",
      "name": "EMOJI MATRIX (07-17)",
      "primary": "#ffaa00",
      "accent": "#ffd700",
      "logoText": "EMOJI MATRIX"
    },
    {
      "preset": "holiday-08-13",
      "name": "LEFT HANDERS DAY (08-13)",
      "primary": "#000080",
      "accent": "#4169e1",
      "logoText": "LEFT HANDERS DAY"
    },
    {
      "preset": "holiday-08-26",
      "name": "DOG DAY (08-26)",
      "primary": "#3e1f00",
      "accent": "#8b4513",
      "logoText": "DOG DAY"
    ...
    {
      "preset": "holiday-09-21",
      "name": "WORLD PEACE DAY (09-21)",
      "primary": "#4682b4",
      "accent": "#ffffff",
      "logoText": "WORLD PEACE DAY"
    },
    {
      "preset": "holiday-10-16",
      "name": "WORLD FOOD DAY (10-16)",
      "primary": "#b8860b",
      "accent": "#d4af37",
      "logoText": "WORLD FOOD DAY"
    },
    {
      "preset": "holiday-11-19",
      "name": "TOILET DAY (11-19)",
      "primary": "#000080",
      "accent": "#4169e1",
      "logoText": "TOILET DAY"
    },
    {
      "preset": "holiday-12-01",
      "name": "AIDS AWARENESS (12-01)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "AIDS AWARENESS"
    },
    {
      "preset": "holiday-12-03",
      "name": "INTERNATIONAL DAY OF PERSONS WITH DISABILITIES (12-03)",
      "primary": "#000080",
      "accent": "#4169e1",
      "logoText": "INTERNATIONAL DAY OF PERSONS WITH DISABILITIES"
    },
    {
      "preset": "holiday-12-05",
      "name": "VOLUNTEER DAY (12-05)",
      "primary": "#b22222",
      "accent": "#ff0000",
      "logoText": "VOLUNTEER DAY"
    },
    {
      "preset": "holiday-04-13",
      "name": "WATER SPLASHING FESTIVAL (04-13)",
      "primary": "#000080",
      "accent": "#00bfff",
      "logoText": "WATER SPLASHING FESTIVAL"
    },
    {
      "preset": "holiday-07-15",
      "name": "NADAM FAIR (07-15)",
      "primary": "#000080",
      "accent": "#0000ff",
      "logoText": "NADAM FAIR"
    },
    {
      "preset": "holiday-11-28",
      "name": "GIVE THANKS (11-28)",
      "primary": "#8b4513",
      "accent": "#ff6347",
      "logoText": "GIVE THANKS"
    },
    {
      "preset": "holiday-04-24",
      "name": "CHINA SPACE (04-24)",
      "primary": "#000080",
      "accent": "#ff0000",
      "logoText": "CHINA SPACE"
    },
    {
      "preset": "holiday-05-17",
      "name": "FOODIE FESTIVAL (05-17)",
      "primary": "#ff4500",
      "accent": "#ff8c00",
      "logoText": "FOODIE FESTIVAL"
    },
    {
      "preset": "holiday-06-18",
      "name": "618 CARNIVAL (06-18)",
      "primary": "#cc0044",
      "accent": "#ff0055",
      "logoText": "618 CARNIVAL"
    },
    {
      "preset": "holiday-12-12",
      "name": "12.12 FIESTA (12-12)",
      "primary": "#0044cc",
      "accent": "#00e5ff",
      "logoText": "12.12 FIESTA"
    },
    {
      "preset": "holiday-07-23",
      "name": "SUMMER SPRITE (07-23)",
      "primary": "#008a3d",
      "accent": "#00ff64",
      "logoText": "SUMMER SPRITE"
    },
    {
      "preset": "holiday-03-21",
      "name": "DEEP SLEEP (03-21)",
      "primary": "#191970",
      "accent": "#9370db",
      "logoText": "DEEP SLEEP"
    },
    {
      "preset": "holiday-04-23",
      "name": "WORLD BOOK DAY (04-23)",
      "primary": "#8b4513",
      "accent": "#d2b48c",
      "logoText": "WORLD BOOK DAY"
    },
    {
      "preset": "holiday-01-28",
      "name": "PRIVACY MATRIX (01-28)",
      "primary": "#1f2937",
      "accent": "#9ca3af",
      "logoText": "PRIVACY MATRIX"
    },
    {
      "preset": "holiday-02-29",
      "name": "LEAP MATRIX (02-29)",
      "primary": "#006400",
      "accent": "#32cd32",
      "logoText": "LEAP MATRIX"
    },
    {
      "preset": "holiday-04-12",
      "name": "ORBITAL MATRIX (04-12)",
      "primary": "#4b0082",
      "accent": "#ff8c00",
      "logoText": "ORBITAL MATRIX"
    },
    {
      "preset": "holiday-04-25",
      "name": "PENGUIN MATRIX (04-25)",
      "primary": "#000080",
      "accent": "#00ffff",
      "logoText": "PENGUIN MATRIX"
    },
    {
      "preset": "holiday-05-16",
      "name": "TERIMA KASIH CIKGU (05-16)",
      "primary": "#8b4513",
      "accent": "#ff6347",
      "logoText": "TERIMA KASIH CIKGU"
    },
    {
      "preset": "holiday-07-07",
      "name": "CHOCO MATRIX (07-07)",
      "primary": "#3e1f00",
      "accent": "#8b4513",
      "logoText": "CHOCO MATRIX"
    },
    {
      "preset": "holiday-07-20",
      "name": "APOLLO MATRIX (07-20)",
      "primary": "#333333",
      "accent": "#e0e0e0",
      "logoText": "APOLLO MATRIX"
    },
    {
      "preset": "holiday-09-09",
      "name": "9.9 SUPER SALE (09-09)",
      "primary": "#ff0000",
      "accent": "#ff4500",
      "logoText": "9.9 SUPER SALE"
    },
    {
      "preset": "holiday-09-19",
      "name": "AHOY MATRIX (09-19)",
      "primary": "#000000",
      "accent": "#daa520",
      "logoText": "AHOY MATRIX"
    },
    {
      "preset": "holiday-10-10",
      "name": "MINDFUL MATRIX (10-10)",
      "primary": "#8a2be2",
      "accent": "#87cefa",
      "logoText": "MINDFUL MATRIX"
    },
    {
      "preset": "holiday-12-26",
      "name": "BOXING DAY (12-26)",
      "primary": "#8b0000",
      "accent": "#ff0000",
      "logoText": "BOXING DAY"
    },
    {
      "preset": "holiday-06-21",
      "name": "SUMMER SOLSTICE (06-21)",
      "primary": "#ff0000",
      "accent": "#ff8c00",
      "logoText": "SUMMER SOLSTICE"
    },
    {
      "preset": "holiday-08-19",
      "name": "CAPTURE MATRIX (08-19)",
      "primary": "#333333",
      "accent": "#c8c8c8",
      "logoText": "CAPTURE MATRIX"
    },
    {
      "preset": "holiday-09-29",
      "name": "COFFEE TIME (09-29)",
      "primary": "#4a2511",
      "accent": "#a0522d",
      "logoText": "COFFEE TIME"
    },
    {
      "preset": "holiday-11-07",
      "name": "WINTER BEGINS (11-07)",
      "primary": "#4682b4",
      "accent": "#87ceeb",
      "logoText": "WINTER BEGINS"
    },
    {
      "preset": "holiday-12-24",
      "name": "SILENT NIGHT (12-24)",
      "primary": "#000080",
      "accent": "#ffd700",
      "logoText": "SILENT NIGHT"
    },
    {
      "preset": "holiday-08-31",
      "name": "MERDEKA (08-31)",
      "primary": "#000080",
      "accent": "#ffd700",
      "logoText": "MERDEKA"
    ...
    {
      "preset": "holiday-09-16",
      "name": "MALAYSIA DAY (09-16)",
      "primary": "#ffd700",
      "accent": "#000080",
      "logoText": "MALAYSIA DAY"
    },
    {
      "preset": "holiday-11-11",
      "name": "11.11SHOPPING!!! (11-11)",
      "primary": "#ff0055",
      "accent": "#ff0055",
      "logoText": "11.11SHOPPING!!!"
    },
    {
      "preset": "holiday-12-31",
      "name": "COUNTDOWN (12-31)",
      "primary": "#ffd700",
      "accent": "#c0c0c0",
      "logoText": "COUNTDOWN"
    },
    {
      "preset": "holiday-l01-09",
      "name": "JADE EMPEROR (L01-09)",
      "primary": "#ff0000",
      "accent": "#ffd700",
      "logoText": "JADE EMPEROR"
    },
    {
      "preset": "holiday-12-22",
      "name": "WINTER SOLSTICE (12-22)",
      "primary": "#4682b4",
      "accent": "#87cefa",
      "logoText": "WINTER SOLSTICE"
    },
    {
      "preset": "holiday-02-01",
      "name": "WILAYAH DAY (02-01)",
      "primary": "#0000ff",
      "accent": "#ffd700",
      "logoText": "WILAYAH DAY"
    },
    {
      "preset": "holiday-05-09",
      "name": "KAMEHAMEHA (05-09)",
      "primary": "#ff4500",
      "accent": "#ff8c00",
      "logoText": "KAMEHAMEHA"
    },
    {
      "preset": "holiday-06-15",
      "name": "KING OF FRUITS (06-15)",
      "primary": "#228b22",
      "accent": "#32cd32",
      "logoText": "KING OF FRUITS"
    },
    {
      "preset": "holiday-09-02",
      "name": "COCO VIBES (09-02)",
      "primary": "#8b4513",
      "accent": "#2e8b57",
      "logoText": "COCO VIBES"
    },
    {
      "preset": "holiday-09-22",
      "name": "SHIRE MATRIX (09-22)",
      "primary": "#006400",
      "accent": "#228b22",
      "logoText": "SHIRE MATRIX"
    },
    {
      "preset": "holiday-11-23",
      "name": "WIBBLY WOBBLY (11-23)",
      "primary": "#001f3f",
      "accent": "#003b6f",
      "logoText": "WIBBLY WOBBLY"
    },
    {
      "preset": "holiday-12-15",
      "name": "TEH TARIK BOSS (12-15)",
      "primary": "#8b4513",
      "accent": "#d2b48c",
      "logoText": "TEH TARIK BOSS"
    },
    {
...
# 来源 说明   1🌙 农历节日20+ 个中国传统节日（春节/元宵/端午/七夕/中秋等），每个有专属特效主题 2📅 公历节日元旦/情人节/Pi Day/圣帕特里克节/愚人节等 3🍗 Crazy Thu每周四特殊覆盖 4👥 社区池用户投稿主题每 3 天轮入一次（约 30% 天数），固定频率不受投稿量影响 5🎲 DailyPool147 预设按 dayOfYear % poolLength 轮换，全年无空档
...
字段 类型 说明   datestringISO 日期 YYYY-MM-DD presetstring主题预设标识符 presetNamestring主题可读名称 authorstring主题作者（"themedist" 或社区作者名） availablenumber可用主题总数 cssVarsobjectCSS 自定义属性键值对（已完成 XSS 清洗） customCssstring主题专属 CSS（已完成 XSS 清洗） extensionsarrayHTML 装饰片段数组（已完成 XSS 清洗） directoryarray主题目录列表 _metaobject安全元信息：sanitized（是否已清洗）、schemaValid（结构校验）、timestamp（处理时间）
...
获取列表 curl — 最新（分页）curl https://themedist.netlify.app/api/v1/diy/themes.json?sort=new&page=1&size=20复制 curl — 最热curl https://themedist.netlify.app/api/v1/diy/themes.json?sort=hot复制 curl — 按标签筛选curl https://themedist.netlify.app/api/v1/diy/themes.json?tag=dark复制 参数: sort=new|hot, page=1, size=20(max50), tag=dark|light|warm|cool|vibrant|minimal|nature|tech 获取单个主题 curlcurl https://themedist.netlify.app/api/v1/diy/theme.json?id=abc12345复制 提交主题 curlcurl -X POST https://themedist.netlify.app/api/v1/diy/submit.json \
  ...
  -d '{"name":"My Theme","author":"You","cssVars":{"--color-primary":"#ff6b6b","--color-bg":"#1a1a2e"},"customCss":"body { ... }","extensions":[{"type":"floating","char":"✨"}],"tags":["dark","vibrant"]}'复制 必填: name, author, cssVars（必须含 --color-primary 和 --color-bg）。可选: customCss, extensions, tags（最多 5 个）。提交后进入审核队列。 成功响应 201：{ success: true, theme: { id, name, ... }, warnings?: [...] }。如果有 warnings 数组，说明部分字段被自动调整（如不支持的类型被移除）。 点赞 curlcurl -X POST https://themedist.netlify.app/api/v1/diy/like.json \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc12345"}'复制 服务器基于 IP + User-Agent 指纹去重，同一用户对同一主题仅计一次。返回 {likes: number, voted: boolean}。
